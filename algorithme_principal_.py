        ...
        if v.get('label') == 1:  # ppd.get('label') is 0:
            ...
            if v.get("ratio") >= 1 and v.get("Area") > 15:
                ...
                for j in range(len(new_cnt)):
                    ...
                    ppd = pd.DataFrame(v, index=[0])
                    #p = train.decision(ppd)
                    if v.get('label') == 1 and v.get("Area") >= 5:  # ppd.get('label') is 0:
                        #print("is face")
            ...
            if v.get("ratio") < 1 and v.get("Area") > 15:
                ...
                for j in range(len(cnt_adjacent)):
                  ...
                  if len(cnt_adjacent[j])<5:
                    ...
                    ppd = pd.DataFrame(v, index=[0])
                    p = v.get("label")
                    if p==1 and v.get('Area') > 3:  # ppd.get('label') is 0:
...
def algorithme(cnt, cnt_int,pretraitement_1, shape, faces=Face(), methode="0", j=0, i=0):
    for i in range(len(cnt)):
        ...
        #print(v.get("label"))
        #print(p[0]==0.0)
        if p[0]==0.0:#v.get('label') == 1:  # ppd.get('label') is 0:
            # print("is face")
            faces.add(dict(contour=cnt[i], prediction=ppd, methode="0", indice=i))
            # i+=1
        else:
            ...
            if v.get("ratio") >= 1 and v.get("Area") > 15 :
                ...
                for j in range(len(new_cnt)):
                    ...
                    ppd = pd.DataFrame(v, index=[0])
                    p = train.decision(ppd)
                    if p[0]==0.0 and v.get('Area') >= 5:  # ppd.get('label') is 0:
                        #print("is face")
                        faces.add(dict(contour=new_cnt[j], prediction=ppd, methode="2", indice=j))
                #cv2.waitKey(0)
                # algorithme( new_cnt, shape,faces, methode + "2", i)
            ...
            if v.get("ratio") < 1 and v.get("Area") > 15 :
                ...
                for j in range(len(cnt_adjacent)):
                  ...
                  if len(cnt_adjacent[j])<5:
                    ...
                    ppd = pd.DataFrame(v, index=[0])
                    p = train.decision(ppd)
                    if p[0]==0.0  and v.get('Area') > 3:#v.get('label') == 1:  # ppd.get('label') is 0:
                        #print("is face")
                        faces.add(dict(contour=cnt_adjacent[j], prediction=ppd, methode="3", indice=j))
            ...
            if v.get("ratio") < 1 and v.get("Area") > 15  :
                ...
                for j in range(len(cnt_adjacent)):
                  ...
                  if len(cnt_adjacent[j])<5:
                    ...
                    ppd = pd.DataFrame(v, index=[0])
                    p = train.decision(ppd)
                    if p[0]==0.0  and v.get('Area') > 9:#v.get('label') == 1:  # ppd.get('label') is 0:
                        #print("is face")
...
def get_ID(path):
    ID = re.sub(r'.*FDDB_FDDB_S/', r'', path)
...
def methode1_phase1(img_prete0, write_me=False):
    # argv[2]+"\\faces\\"+str(argv[3])
    # pathImgToWrite="training_3/"

    img_prete = img_prete0.copy()
    img_inv, img_prete, external_region, internal_region = split_region(img_prete)
...
def split_region(img, write_me=False):
    ...
    if write_me:
        cv2.imwrite("split_region.bmp",np.hstack((img_inv,img_prete, mask_extern, mask_internal)))
...
def regrouping(argv, imgb_, img_colored_):
    faces = Face()
    #print("ç")
    #print(faces.faces)
    #print("ç")
    ...
    img_inv, img_prete, mask_extern, mask_internal = methode1_phase1( img)
    ...
    if argv[0]=="0":
        algorithme_methode(cnt, mask_internal,mask_extern, img.shape, faces)
    else:
        if argv[0]=="1":
            algorithme(cnt, mask_internal,mask_extern, img.shape, faces)
        else:
            if argv[0]=="2":
                algorithme_haar(img_colored.copy(), faces)

    for i in range(len(faces.faces)):
        faces.draw_faces(img_colored, i)
    #cv2.imwrite("result/"+argv[1]+argv[0]+".jpg", img_colored)
    t_fin=time.time()
    print("time :"+str(argv[0])+" : "+str(t_fin-t_deb))

    faces.delet_attribute()
    del faces
    return img_colored,"time :"+str(methode_chosen.get(argv[0]))+" : "+str(t_fin-t_deb)
