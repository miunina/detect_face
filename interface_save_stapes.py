...
def main():
    ...
    def clicRechercher():
        ...
        if len(filepath) > 0 and isinstance(img_, Image_.Image_):
            ...
            w = 200
            gray_show = cv2.resize(gray_show, (w, int((w / gray_show.shape[1]) * gray_show.shape[0])), 0, 0,
                                   cv2.INTER_NEAREST)
    ...
    EBM =  BooleanVar()
    IBM =  BooleanVar()
    mframeSeuilage=Menu(frameSeuilage)

    R = Menu(mframeSeuilage)
    R.add_checkbutton(lable="SEGMENTED", fg="gray99", bg="gray9"
                      , onvalue=True, offvalue=0,variable=SEG)
        #.pack(anchor=W)
    R.add_checkbutton(lable="BINARY ORIGINAL", fg="gray99", bg="gray9",
                      onvalue=True, offvalue=0,variable=BNO)
        #.pack(anchor=W)
    R.add_checkbutton(lable="BINARY INVERSE", fg="gray99", bg="gray9",
                     onvalue=True, offvalue=0, variable=BNI)
        #.pack(anchor=W)

    R.add_checkbutton(label="external binary image",  fg="gray99", bg="gray9" ,
                     onvalue=True, offvalue=0,variable=EBM)
    #.pack(anchor=W) \
    R.add_checkbutton(label="internal binary image",
                     onvalue=True, offvalue=0, variable=IBM)
    #.pack(anchor=W)
    mframeSeuilage.add_cascade(label='view',menu=R)
    frameSeuilage.config(menu=mframeSeuilage)
    PReglage.add(frameSeuilage)
    PReglage.pack()
